...
- 📸 **Instagram Фото, Видео и Карусели**: Загрузка одиночных фотографий и роликов в максимальном качестве, а также многостраничных постов (каруселей) с интерактивным окном выбора снимков и видео ("ВЫБРАТЬ ВСЕ", превью, номера элементов).
- 🍪 **Импорт Cookies**: Авторизация через Chrome, Edge, Firefox, Brave, Opera, Яндекс для 18+ и приватных видео.
- 📦 **Универсальная очередь (Unified Queue)**: Пакетная загрузка смешанного списка интернет-ссылок и локальных файлов в одном интерфейсе с независимыми настройками для каждого элемента.
- 🛡️ **Атомарная запись и защита файлов**: Запись во временные `.part`/`.tmp` файлы исключает повреждение данных; умный алгоритм `get_unique_path` защищает от перезаписи существующих файлов.
...
- ⚡ **AI Повышение плавности (60 / 120 FPS)**: Интерполяция кадров нейросетью RIFE (Vulkan GPU) с проверкой целостности SHA-256 и безопасным хранением в `%LOCALAPPDATA%`.
- 📂 **Local Studio (Drag & Drop)**: Перетаскивание локальных видеофайлов и папок с компьютера для моментальной обработки.
- 💬 **Адаптивное сжатие для Discord (< 8 МБ)**: Автоматический двухпроходный расчет битрейта и динамическое масштабирование разрешения для гарантированного соблюдения лимита 8 МБ при высоком качестве.
- 🛑 **Отзывчивая отмена**: Мгновенная остановка фоновых процессов FFmpeg и yt-dlp без зависания UI и утечки системных ресурсов.
...
- **Медиа-движок**: [FFmpeg](https://ffmpeg.org/) и FFprobe (для объединения потоков, нарезки, кадрирования и сжатия)
  - *Способ 1 (через winget)*:
    ...
    ```
  - *Способ 2 (портативный)*: просто поместите `ffmpeg.exe` и `ffprobe.exe` в папку `tools/` рядом с программой — Aura Downloader автоматически обнаружит их без изменения системного PATH.
- **AI Интерполяция (RIFE)**: Любая современная видеокарта NVIDIA / AMD / Intel с поддержкой Vulkan API.
